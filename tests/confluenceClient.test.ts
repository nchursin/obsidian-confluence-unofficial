...
describe('ConfluenceClient', () => {
  ...
  describe('upsertPage', () => {
    it('should POST to /rest/api/content for new page', async () => {
      mockAuth.getAuth.mockReturnValue({ type: 'BASIC', basic: { username: 'u', token: 't' } });
      mockRequest.mockResolvedValue('ok');
      await expect(client.upsertPage(basePage)).resolves.toBe('ok');
      const call = mockRequest.mock.calls[0][0];
      expect(call.url).toBe('https://example.atlassian.net/rest/api/content');
    ...
    it('should PUT to /rest/api/content/{pageId} for existing page', async () => {
      ...
      await expect(client.upsertPage(page)).resolves.toBe('ok');
      const call = mockRequest.mock.calls[0][0];
      expect(call.url).toBe('https://example.atlassian.net/rest/api/content/456');
    ...
    it('should include all required headers', async () => {
      mockAuth.getAuth.mockReturnValue({ type: 'PAT', bearer: { token: 'tok' } });
      mockRequest.mockResolvedValue('ok');
  ...
  describe('getAuthHeader (indirect via upsertPage)', () => {
    it('should throw if BASIC username is missing', async () => {
      mockAuth.getAuth.mockReturnValue({ type: 'BASIC', basic: { token: 't' } });
      await expect(client.upsertPage(basePage)).rejects.toThrow(/Username/);
    });
    it('should throw if BASIC token is missing', async () => {
      mockAuth.getAuth.mockReturnValue({ type: 'BASIC', basic: { username: 'u' } });
      await expect(client.upsertPage(basePage)).rejects.toThrow(/Password/);
    });
